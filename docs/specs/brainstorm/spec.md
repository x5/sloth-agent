...
> 最后更新: 2026-05-03 (iter6-doc-sync)
> Iter-4 完成: 2026-05-01
> Iter-5 完成: 2026-05-01
> Iter-6 完成: 2026-05-02
> Scope: Core（Desktop 首发 adapter，CLI adapter 后续接入）
...
分层约束：
- Brainstorm Core（复用目标）：轮次推进、发言裁剪、回复链保护、投票/收敛策略。
- Desktop Adapter（当前实现）：SSE 推送、UI 线程渲染、sidecar 存储。
- CLI Adapter（后续实现）：终端流式展示、命令触发与脚本化控制。
...
**彩色线程引用预览：**
- `threadColor.ts`：`getRootId(id, messages)` → FNV-1a hash → 固定调色板索引 → 稳定线程 accent color

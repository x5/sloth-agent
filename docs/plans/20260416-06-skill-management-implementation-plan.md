...
v0.1 实现 SKILL.md 加载与按需注入机制：从文件系统扫描、解析 frontmatter、提供技能内容供 prompt 注入。不实现 skill_router、自动匹配、向量检索（v0.3+）。
...
v0.2 在 Chat REPL 中增加 `/skills` 命令，扫描 `local_skills/` 目录并列出可用 skill。
...
> 依赖: v0.1/v0.2 已完成基础加载和 REPL 列出能力

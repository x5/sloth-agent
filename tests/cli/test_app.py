...
def test_run_command_calls_pipeline(tmp_path):
    """Mock Orchestrator and Runner, verify run command calls pipeline."""
    ...
    with patch("sloth_agent.core.config.load_config") as mock_load_config, \
         patch("sloth_agent.core.orchestrator.ProductOrchestrator") as mock_orch_cls, \
         patch("sloth_agent.core.runner.Runner") as mock_runner_cls:
        mock_config = MagicMock()
        mock_load_config.return_value = mock_config

        mock_orchestrator = MagicMock()
        mock_orchestrator.create_run_state.return_value = mock_state
        mock_orchestrator.tool_registry = MagicMock()
        ...
        mock_orchestrator.create_run_state.assert_called_once()
